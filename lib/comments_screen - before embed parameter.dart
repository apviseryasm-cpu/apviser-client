...
class CommentsScreen extends StatefulWidget {
  ...
  CommentsScreen({required this.questionId});
...
class _CommentsScreenState extends State<CommentsScreen> {
  ...
  void _processCommentsResponse(Map<String, dynamic> data, int parentId) {
    ...
    if (data['comments'] != null && data['comments'].isNotEmpty) {
      if (parentId == 0) {
        fetchedComments = (data['comments'] as List).map((e) => CommentDTO.fromJson(e)).toList();
      } else {
        var parentComment = data['comments'][0];
        if (parentComment['comments'] != null) {
          fetchedComments = (parentComment['comments'] as List).map((e) => CommentDTO.fromJson(e)).toList();
        }
      }
    }
    ...
    bool addReply(List<CommentDTO> commentsList) {
      for (var comment in commentsList) {
        if (comment.ID == parentId) {
          ...
          final uniqueReplies = newReplies.where((c) => !existingIds.contains(c.ID)).toList();
  ...
  void _toggleReplies(CommentDTO comment) {
    ...
    if (comment.isExpanded && comment.children!.isEmpty) {
      ...
      _fetchComments(parentId: comment.ID!).then((_) => setState(() => comment.isLoadingReplies = false));
    }
  }
  ...
  Future<void> _submitComment(int parentId, {CommentDTO? editingComment}) async {
    String replyText = _commentController.text.trim(); // Capture text before clearing
    ...
    try {
      ...
      if (response.statusCode == 200 && responseData["error"] == false) {
        ...
        _showSnackBar(editingComment == null ? 'Comment posted successfully!' : 'Comment updated successfully!');
      } else {
        _showSnackBar('Failed to ${editingComment == null ? 'post' : 'update'} comment. Try again!');
      }
    } catch (e) {
      _showSnackBar('Error ${editingComment == null ? 'posting' : 'updating'} comment.');
    } finally {
  ...
  void _updateCommentInList(List<CommentDTO> commentList, CommentDTO updatedComment) {
    for (int i = 0; i < commentList.length; i++) {
          ...
          Expanded(
            child: _isLoading
              ...
              itemBuilder: (context, index) => CommentWidget(
                ...
                onEditComment: (comment) => _editComment(comment), // Pass edit function
              ),
            ),
          ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: _isSubmittingComment ? const CircularProgressIndicator() : const Icon(Icons.send, color: Colors.blue),
    ...
    showModalBottomSheet(context: context, builder: (context){
      return Padding(padding: const EdgeInsets.all(8.0),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: _commentController, decoration: const InputDecoration(hintText: 'Edit comment...')),
...
class _CommentWidgetState extends State<CommentWidget> {
  ...
  void initState() {
    super.initState();
    _editController.text = widget.comment.descr!; // Initialize with existing text
  }
        ...
        Row(
          ...
          children: [
            if (widget.level > 0)
              SizedBox(
                width: 30,
                child: CustomPaint(size: const Size(30, 50), painter: TreeLinePainter(isLastChild: widget.isLastChild)),
              ),
            Expanded(
              child: ListTile(
                ...
                title: Text(widget.comment.commentatorName!, style: const TextStyle(fontWeight: FontWeight.bold)),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    if (_isEditing)
                      Row(
                        children: [
                          ...
                          IconButton(
                          onPressed: () {
                          _updateComment(context); // Call update function
                          },
                          icon: const Icon(Icons.refresh, color: Colors.blue,), // Use refresh icon
                          ),
                          IconButton(onPressed: (){setState(() {_isEditing = false;});}, icon: const Icon(Icons.cancel, color: Colors.blue,))
                        ],
                      )
                    else
                      ...
                      children: [
                        ...
                        TextButton(
                          ...
                          child: Text(widget.comment.isExpanded
                              ? 'Hide Replies'
                              : widget.comment.totalComments! > 0 ? 'View Replies (${widget.comment.totalComments})' : 'View Replies'),
                        ),
                        TextButton(
                        ...
                        if (widget.cuserID == widget.comment.userID) ...[
                          ...
                          IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _showDeleteConfirmation(context, widget.comment)),
                        ],
                      ],
                    ),
              ...
              child: Column(
                children: widget.comment.children!.asMap().entries.map((entry) {
                  ...
                  return CommentWidget(
                    ...
                    onEditComment: widget.onEditComment, // Pass edit function here!
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && responseData["error"] == false) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to toggle like. Try again!')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error toggling like.')));
    }
  }

  void _updateComment(BuildContext context) {
    final parentState = context.findAncestorStateOfType<_CommentsScreenState>()!;
    parentState._submitComment(widget.comment.ID!, editingComment: widget.comment.copyWith(descr: _editController.text));
    setState(() {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: _isSubmittingReply ? const CircularProgressIndicator() : const Icon(Icons.send, color: Colors.blue),
    ...
    try {
      ...
      if (response.statusCode == 200 && responseData["error"] == false) {
        ...
        setState(() {
          ...
          (context.findAncestorStateOfType<_CommentsScreenState>())?._addReplies(widget.comment.ID!, [newComment]);
        });
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reply posted successfully!')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to post reply. Try again!')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error posting reply.')));
    } finally {
  ...
  Future<void> _deleteComment(CommentDTO comment) async {
    final payload = [
      {"tbl": "comments_and_likes", "key_values": [{"key": "ID", "value": comment.ID}]},
      [{"key": "status", "value": 0}]
    ];
    ...
    try {
      ...
      if (response.statusCode == 200 && responseData['error'] == false) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(responseData['error_msg'] ?? "Failed to delete comment")));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Error deleting comment")));
    }
  }
